...
// `pendienteReal` admite boolean (piloto: EU-DEM real -> true) o un string de
// nivel ("aproximacion" para SRTM 30 m de respaldo a escala, que NO es el
// MDT-CNIG de referencia y por eso no se cuenta como "real").
export function confianzaNucleo({ edadReal, poblacionReal, capacidadReal, pendienteReal, combustible, socialCompleto = true }) {
  ...
  const nivelPendiente = typeof pendienteReal === "string"
    ? (CONFIANZA_NIVEL[pendienteReal] ?? CONFIANZA_NIVEL.estimacion)
    : nivel(pendienteReal);
  const social = [
  ...
  const confPeligro = PESO_PENDIENTE * nivelPendiente + PESO_COMBUST * nivelCombustible;
