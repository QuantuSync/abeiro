...
// Nivel por variable: real = 1, aproximación = 0.6, estimación = 0.3. Se
// promedia por componente (con los subpesos de cada componente) y se pondera
// por los pesos del IV:
...
// -----------------------------------------------------------------------------
export const CONFIANZA_NIVEL = { real: 1, aproximacion: 0.6, estimacion: 0.3 };

export function confianzaNucleo({ edadReal, poblacionReal, capacidadReal, pendienteReal, combustibleAprox }) {
  const nivel = (real, aprox = false) =>
    real ? CONFIANZA_NIVEL.real : aprox ? CONFIANZA_NIVEL.aproximacion : CONFIANZA_NIVEL.estimacion;
  const confSocial =
    SUBPESOS_SOCIAL.mayores_65 * nivel(edadReal)
    + SUBPESOS_SOCIAL.hogares_unipersonales * CONFIANZA_NIVEL.estimacion // Fase 0
    + SUBPESOS_SOCIAL.dispersion * CONFIANZA_NIVEL.estimacion            // Fase 0
    + SUBPESOS_SOCIAL.poblacion * nivel(poblacionReal);
  const confPeligro =
    PESO_PENDIENTE * nivel(pendienteReal)
    + PESO_COMBUST * nivel(false, combustibleAprox);
